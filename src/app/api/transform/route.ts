...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      case "transform": {
        const { content: rawContent, sourceContent, outputTypes: rawOutputTypes, config: rawConfig, mediaTypes: rawMediaTypes } = body;
        const mediaTypes = Array.isArray(rawMediaTypes) && rawMediaTypes.length > 0
          ? rawMediaTypes
        ...
        const sentences = inputContent
          .split(/[.!?\n]+/)
          .filter((s: string) => s.trim().length > 10);
        let videoScenes: Array<{ sceneNumber: number; imageUrl: string; narration: string; duration: number }> = [];
        if (mediaTypes.includes('video')) {
          videoScenes = Array.from({ length: 4 }, (_, i) => {
        ...
        if (mediaTypes.includes('presentation')) {
          try {
        ...
        return NextResponse.json({
          ...
          // Additional media
          media: {
            image: imageUrl ? { url: imageUrl, prompt: mediaPrompt } : null,
            video: videoScenes.length > 0 ? { scenes: videoScenes, totalDuration: videoScenes.length * 4000 } : null,
